...
class ShortRestPhysics(Physics):
    def reset(self, cmd: Command):
        super().reset(cmd)
        self.rest_duration = 500  # 1.5 sec
        self.start_time = None
        self.start_cell = tuple(cmd.params[0])
        self.pos = self.board.cell_to_world(self.start_cell)
...
class JumpPhysics(Physics):
    def reset(self, cmd: Command):
        super().reset(cmd)
        self.finished = False   # <<< מאפס כדי לאפשר קפיצה מחדש
        self.jump_duration = 1000  # משך הקפיצה במילישניות
        self.start_time = None
        ...
        self.pos = self.start_pos
        self.jump_height = 20  # כמה פיקסלים לעלות בקפיצה
    ...
    def update(self, now_ms: int) -> Command:
        ...
        if t >= 1.0:
            # בסוף הקפיצה מחזירים למיקום היעד
            self.pos = self.end_pos
            self.finished = True
            return Command(
                timestamp=now_ms,
        ...
        # תנועה ליניארית קדימה + פרבולה למעלה/למטה
        base_x = self.start_pos[0] + t * (self.end_pos[0] - self.start_pos[0])
        base_y = self.start_pos[1] + t * (self.end_pos[1] - self.start_pos[1])

        y_offset = -4 * self.jump_height * t * (1 - t)
        self.pos = (base_x, base_y + y_offset)
        return None
